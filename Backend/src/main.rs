...
async fn main() {
    ...
    let listener = tokio::net::TcpListener::bind(format!("0.0.0.0:{}", 8001))
        .await
        .unwrap();
...
pub async fn upload_chunk(mut multipart: Multipart) -> impl IntoResponse {
    ...
    if file_name.is_empty() || chunk_data.is_empty() {
        eprintln!(
            "Missing required fields: file_name: {}, chunk_data: {:?}",
            file_name, chunk_data
        );
        return StatusCode::BAD_REQUEST;
    }
    ...
    let mut file = match File::create(&chunk_path) {
        ...
        Err(err) => {
            eprintln!(
                "Failed to create chunk file: {:?}, Error: {:?}",
                chunk_path, err
            );
            return StatusCode::INTERNAL_SERVER_ERROR;
        }
    ...
    if let Err(err) = file.write_all(&chunk_data) {
        eprintln!(
            "Failed to write chunk data: {:?}, Error: {:?}",
            chunk_path, err
        );
        return StatusCode::INTERNAL_SERVER_ERROR;
    }
